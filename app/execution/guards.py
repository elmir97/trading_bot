...
def check_no_existing_position(*, has_open_position: bool) -> ExecutionRefusal | None:
    """has_open_position считается ПО СИМВОЛУ (app/execution/service.py),
    сторона сигнала НАМЕРЕННО не участвует — не пропуск, чинить не надо
    (раздел 16 ТЗ, шаг 15.5.1, п.4 разведки; замок поведения —
    test_existing_position_refuses_opposite_side_too). Замысел гварда —
    "не иметь двух позиций по одному инструменту", а не "не иметь двух
    одинаковых":
      ...
      - в хедже встречный вход при уже открытой позиции — это
        ...
        сознательно не делаем); это разворот, а разворот требует сначала
        закрыть позицию, чего в этот этап нет.
    Сравнение по (symbol, side) вместо символа сломало бы оба случая."""
