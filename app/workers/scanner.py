...
коммит — после каждого слота (_scan_user), не один на весь run(): иначе
кнопка, нажатая до конца прохода, не находит снимок («устарело»), а «Да»
по живому уведомлению ждёт блокировку строки слота, которую держит
незакоммиченный UPDATE сканера. Если коммит не случится уже после
отправки, кнопка укажет на id, которого в БД нет, — хендлер ответит
«Уведомление устарело»; sequence PostgreSQL не откатывается, поэтому этот
id никогда не достанется другому уведомлению. Такое уведомление —
единственное: всё до него уже закоммичено.
"""
    ...
    async def _scan_user(self, session: AsyncSession, user: User) -> int:
        ...
        for symbol in plan.allowed_symbols:
            ...
            for timeframe in SCAN_TIMEFRAMES:
                ...
                # Слот, погашенный слот другого уровня и снимок уведомления —
                # одна единица, коммитятся вместе. Раньше коммит был один на
                # проход: кнопка до его конца давала «устарело», а «Да» ждало
                # блокировку строки слота весь проход (см. docstring модуля).
